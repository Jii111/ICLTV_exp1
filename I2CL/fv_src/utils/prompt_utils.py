...
class ICLDataset:
    ...
    def __init__(self, dataset):    
        ...
        elif isinstance(dataset, pd.DataFrame):   # ✅ 수정
...
def md_to_load_dataset(task_name: str,
                       train_dataset, valid_dataset, test_dataset) -> Dict[str, ICLDataset]:
    """
    Convert md.get_dataset() outputs (BaseTask style) into FV-style ICLDataset splits.

    Parameters:
    - task_name: dataset name (for logging/debugging only)
    - train_dataset, valid_dataset, test_dataset: md.get_dataset() 결과 객체
    - seed: reproducibility (현재는 미사용, split은 이미 md 단계에서 끝났음)

    Returns:
    dict: {"train": ICLDataset, "valid": ICLDataset, "test": ICLDataset}
    """

    def convert_md_split(md_split):
        converted = []
        for data in md_split.all_data:
            input_str, ans_str, label = md_split.apply_template(data)
            output_str = ans_str[label]  # 정답 텍스트 뽑기
            converted.append({"input": input_str, "output": output_str})
        return ICLDataset(pd.DataFrame(converted))  # FV에서 기대하는 ICLDataset

    train = convert_md_split(train_dataset)
    valid = convert_md_split(valid_dataset)
    test = convert_md_split(test_dataset)

    dataset_dict = {"train": train, "valid": valid, "test": test}

    print(f"[md_to_load_dataset] Converted {task_name}: "
          f"train={len(train)}, valid={len(valid)}, test={len(test)}")
    print(" Example from Train:", train[0]) 
