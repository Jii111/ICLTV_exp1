...
# ✅ 수정
def eval_i2cl_style(dataset_name, dataset, model, tokenizer, model_config,
                    fv_vector=None, edit_layer=None, n_shots=5,
                    prefixes=None, separators=None):

    """
    FV + I2CL style label-level evaluation.
    Returns logits/probs over label options instead of token ranks.
    """
    
    # 1. 라벨 후보 준비
    options = md.get_dataset(dataset_name, split=None).get_demonstration_template()['options']
    label_map = {}
    ...
    clean_logits, fv_logits, all_labels = [], [], []
    pred_labels_clean, pred_labels_fv = [], []
    ...
    for j in tqdm(range(len(dataset['test'])), total=len(dataset['test'])):
        ...
        # FV intervention 실행
        clean_out, fv_out = function_vector_intervention(
        ...
        clean_out = clean_out[:, interest_index]
        fv_out = fv_out[:, interest_index]

        clean_logits.append(clean_out.detach().cpu())
        fv_logits.append(fv_out.detach().cpu())

        gold_label = label_map[tokenizer.encode(" " + target, add_special_tokens=False)[0]]
        all_labels.append(gold_label)

        pred_labels_clean.append(clean_out.softmax(dim=-1).argmax(dim=-1).item())
        pred_labels_fv.append(fv_out.softmax(dim=-1).argmax(dim=-1).item())

    clean_logits = torch.cat(clean_logits, dim=0)
    fv_logits = torch.cat(fv_logits, dim=0)

    # 3. KL divergence 계산
    mean_kl, kl_values = compute_kl_divergence(clean_logits, fv_logits)
    ...
    metrics_clean = compute_metrics(pred_labels_clean, all_labels, len(options))
    metrics_fv = compute_metrics(pred_labels_fv, all_labels, len(options))

    # 5. 결과 정리
    result_dict = {'demon': {},
                   'split_demon': {},
                   'test_result': {'few_shot': [metrics_clean],'ours': [metrics_fv]}, 
                   'linear_coef': {},
    ...
    kl_dict = {
        ...
        'labels': list(map(int, all_labels))
    }
