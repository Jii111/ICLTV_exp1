...
if __name__ == "__main__":
    ...
    parser.add_argument('--root_data_dir', help='Root directory of data files', type=str, required=False, default='/home/jiii111/iclTaskVector/function_vectors/dataset_files')
    parser.add_argument('--save_path_root', help='File path to save to', type=str, required=False, default='/home/jiii111/iclTaskVector/function_vectors/results')
    ...
    # ✅ 수정
    parser.add_argument("--test_data_num", help="test_data_num", type=int, required=True) 
    parser.add_argument("--val_data_num", help="valid_data_num", type=int, required=True) 
    parser.add_argument('--sample_method', help='sample_method', type=str, required=True) 
    parser.add_argument('--shot_per_class', help='shot_per_class', type=int, required=False) 
    ...
    train_dataset = md.get_dataset(args.dataset_name, split='train',
                    max_data_num=None, seed=args.seed)
    valid_dataset = md.get_dataset(args.dataset_name, split='validation', 
                    max_data_num=args.val_data_num,
                    sample_mode=args.sample_method, seed=args.seed)
    test_dataset = md.get_dataset(args.dataset_name, split='test', 
                    max_data_num=args.test_data_num,
                    sample_mode=args.sample_method, seed=args.seed)
    
    dataset = md_to_load_dataset(dataset_name, 
                                train_dataset, valid_dataset, test_dataset)
    
    #n_shots = args.n_shots # ✅ 수정
    if args.dataset_name  == 'dbpedia':  # always use 1-shot for dbpedia
        args.shot_per_class = 1
        #args.config['bs'] = 1
    n_shots = get_shot_num(train_dataset, args.shot_per_class)
    ...
    # ✅ 수정
    print("Running I2CL-style evaluation")
    test_result, kl_dict = eval_i2cl_style(args.dataset_name,dataset, 
                                    model=model, model_config=model_config, tokenizer=tokenizer,
                                    fv_vector=fv, edit_layer=edit_layer, n_shots=n_shots,
                                    prefixes=prefixes, separators=separators)
    
    with open(args.save_path_root + '/test_result_i2cl_st.json', 'w') as f:
        json.dump(test_result, f, indent=4)
    with open(args.save_path_root + '/kl_divergence.json', 'w') as f:
        json.dump(kl_dict, f, indent=4)
    
    plot_kl_hist(kl_dict['kl_values'], kl_dict['mean_kl'], args.save_path_root + f"/kl_hist.png")
